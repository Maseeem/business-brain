...
def analyze_image_for_knowledge(uploaded_file):
    ...
    response = client.models.generate_content(
        ...
        contents=[
            ...
            (
                "Extract the useful business knowledge visible in this image. "
                "Transcribe relevant text and describe operational details accurately. "
                "Do not invent missing information. "
                "Return clean plain text suitable for a knowledge base."
            ),
        ],
        config=types.GenerateContentConfig(
            temperature=0.1,
...
"""
        ...
        return {
            "ok": True,
            "sop": sop,
            "sources": [f.name for f in uploads or []],
        }
    ...
    try:
        ...
        if not results:
            return {
                "ok": True,
                "answer": (
                    "I couldn't find enough information in your Business Brain "
...
"""
        ...
        return {
            "ok": False,
            "error": f"Business Brain could not answer right now: {e}",
        }
